...
export default function (key: string, data: any, config: IMSLocalStorageConfig = {}) {
  ...
  const tmpMaxAgeObj: Partial<IMSLocalStorageStore> = maxAge ? { maxAge: Date.now() + maxAge * 1000 } : expires ? { maxAge: expires.getTime() } : {};
  return localStorage.setItem(getFullKeyStr(key, config), JSON.stringify({ ...tmpMaxAgeObj, data } as IMSLocalStorageStore));
}
